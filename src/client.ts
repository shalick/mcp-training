        ...
        for (const message of request.params.messages) {
            ...
            for (const content of contents) {
                if (content.type === "text") {
                    const text = await handleTextPrompt((content as any).text)
...
async function handleQuery(tools: Tool[]) {
    ...
    const { text, toolResults } = await generateText({
        ...
        prompt: query,
        tools: tools.reduce<Record<string, any>>((obj, tool) => {
            return {
                ...obj,
                [tool.name]: {
                    description: tool.description || "",
    ...
    console.log(
        text || (toolResults[0]?.output as any)?.content?.[0]?.text || "No text generated."
    )
}
...
async function handleTool(tool: Tool) {
    ...
    if (content && "text" in content) {
        ...
        console.log("Tool returned non-text or empty content.");
    }
}
...
async function handleResource(uri: string) {
    ...
    if (content && "text" in content) {
        ...
        console.log("Resource contains binary data or is empty.");
    }
}
...
async function handleServerMessagePrompt(message: PromptMessage) {
    ...
    return handleTextPrompt(message.content.text)
}
